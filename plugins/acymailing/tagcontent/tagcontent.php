...
class plgAcymailingTagcontent extends JPlugin
{
	...
	<?php
	 ...
	 private function _replaceContent(&$results,$i){
		...
		if($tag->type != "title"){
			...
			if($tag->type == "intro")
			{
				...
				if(empty($article->fulltext)){
					$article->introtext = $acypluginsHelper->wrapText($article->introtext, $mytag);
			...
			if($tag->type == "intro"){
				if(empty($tag->noreadmore) && (!empty($article->fulltext) || $forceReadMore)){
		...
		if(!empty($tag->cattitle) && ((!strpos($article->catid, ',') && $this->currentcatid != $article->catid) || (strpos($article->catid, ',') && !in_array($this->currentcatid, explode(',', $article->catid))))){
		...
		foreach($results as $var => $allresults){
			foreach($allresults[0] as $i => $oneTag){
				...
				if(!empty($parameter->filter) && !empty($email->params['lastgenerateddate'])){
					$condition = '(`publish_up` > \''.date( 'Y-m-d H:i:s',$email->params['lastgenerateddate'] - date('Z')).'\' AND `publish_up` < \''.date('Y-m-d H:i:s',$time).'\')';
					$condition .= ' OR (`created` > \''.date( 'Y-m-d H:i:s',$email->params['lastgenerateddate'] - date('Z')).'\' AND `created` < \''.date('Y-m-d H:i:s',$time).'\')';
					if($parameter->filter == 'modify'){
						$condition .= ' OR (';
						$condition .= ' (`modified` > \''.date( 'Y-m-d H:i:s',$email->params['lastgenerateddate'] - date('Z')).'\' AND modified` < \''.date( 'Y-m-d H:i:s',$time).'\')';
				...
				if(empty($parameter->unpublished)){ $where[] = 'state = 1';}else{$where[] = 'state = 0';}
