...
/**
 * Ok so this is getting a little messy in this file, but that is because we are moving to a SINGLE ENDPOINT with full
 * request validation of all requests we make - soon this will be refactored into its own class but for speed its here
 * right now and working very very well while we iron out the niggles with crappy servers!
 */
...
if (!is_object($dataObj)) {
    die (json_encode(array(
                         ...
                         'RESULT'       => bfReply::ERROR,
                         'NOTENCRYPTED' => array(
                             'msg' => 'Failed Security 101 - This means you need new encryption keys, install a new connector please'
                         )
                     )));
}
