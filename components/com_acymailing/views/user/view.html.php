	...
	function modify(){
		...
		if (is_object( $menu )) {
			...
			if(!empty($menuparams)){
				...
				$autolistsString = strtolower($menuparams->get('autolists', 'none'));
				if($autolistsString != 'none') $autolists = explode(',', $autolistsString);
		...
		if(empty($subscriber)){
			...
			if(!empty($subscription)){
				foreach($subscription as $id => $onesub){
					...
					if(!empty($menuparams) && $autolistsString != 'all' && (empty($autolists) || !in_array($id, $autolists)) && strtolower($menuparams->get('listschecked','all')) != 'all' && !in_array($id,explode(',',$menuparams->get('listschecked','all')))){
		...
		$this->assignRef('autolistsString',$autolistsString);
		if(!empty($autolists)) $this->assignRef('autolists',$autolists);
